...
function loadLocal(): WorkoutState | null {
  try {
    ...
    // must have an exercises array to be a valid state
  ...
  if (!state) {
    return (
      <div className={`min-h-screen ${colors.bg.base} flex items-center justify-center flex-col gap-4`}>
        ...
        <Link href="/workout/start" className={`px-6 py-2.5 rounded-xl font-semibold text-sm ${colors.accent.primary}`}>
          Start One
        </Link>
      </div>
    );
      ...
      <main className="max-w-2xl mx-auto px-4 py-4 space-y-4">
        {state.exercises.map((ex, exIdx) => (
          <div key={exIdx} className={`${colors.bg.elevated} border ${colors.border.default} rounded-2xl overflow-hidden`}>
            ...
            {/* Column headers */}
            <div className={`grid grid-cols-[28px_24px_1fr_12px_1fr_32px] items-center px-3 py-1.5 border-b ${colors.border.muted}`}>
              <div />
              <span className={`text-xs ${colors.text.muted}`}>Set</span>
              <span className={`text-xs ${colors.text.muted} text-center`}>Weight (kg)</span>
              <div />
              <span className={`text-xs ${colors.text.muted} text-center`}>Reps</span>
              <div />
            </div>
            ...
            <div className="divide-y divide-zinc-800/40">
              {ex.sets.map((s, setIdx) => (
                <div
                  key={setIdx}
                  className="grid grid-cols-[28px_24px_1fr_12px_1fr_32px] items-center gap-x-1 px-3 py-2"
                >
                  {/* Done toggle */}
                  <button
                    onClick={() => toggleDone(exIdx, setIdx)}
                    className={`w-7 h-7 rounded-full flex items-center justify-center transition-colors ${colors.interactive.base} ${
                      s.done
                  ...
                  >
                    {s.done && (
                      <svg viewBox="0 0 12 12" className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
                        <polyline points="2,6 5,9 10,3" />
                      </svg>
                    )}
                  </button>

                  <span className={`text-xs ${colors.text.muted} text-center`}>{s.set_number}</span>
                  ...
                  <div className="flex items-center gap-0.5">
                    ...
                    <input
                      ...
                      placeholder="0"
                      className={`flex-1 min-w-0 text-center px-0 py-1.5 rounded-lg ${colors.bg.base} border ${colors.border.default} text-sm focus:outline-none focus:border-zinc-500`}
                    />
                    <button
                  ...
                  <span className={`text-xs ${colors.text.muted} text-center`}>×</span>
                  ...
                  <button
                    ...
                    className={`w-8 h-8 flex items-center justify-center text-lg text-zinc-200 active:text-red-400 ${colors.interactive.base}`}
                  >
                    ✕
                  </button>
                </div>
